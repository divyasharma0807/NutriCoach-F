/**
 * K6 Load Test Scenario - Referrals GET /api/referrals
 * Responsibility: Performs authentication for user accounts, queries referrals lists,
...
import { validateReferralsResponse } from '../utils/responseValidators.js';
import { sleepRandom } from '../utils/helpers.js';
import { generateSummary } from '../utils/reporter.js';
import { referralsDuration, referralsSuccess, referralsFailure, referralsFailureRate } from '../customMetrics/metrics.js';
...
export default function () {
  ...
  if (!account) {
    referralsFailure.add(1);
    referralsFailureRate.add(true);
  ...
  if (!auth.token || !auth.validation.valid) {
    referralsFailure.add(1);
    referralsFailureRate.add(true);
    console.warn(`[REFERRALS ABORT] Login authentication failed for phone ${phone}, skipping iteration.`);
    return;
  }

  const url = `${BASE_URL}/api/referrals`;
  const params = {
  ...
  try {
    ...
    referralsFailure.add(1);
    referralsFailureRate.add(true);
    console.error(`[NETWORK EXCEPTION] GET /referrals failed for Phone: ${phone}. Error: ${err.message}`);
    return;
  }

  const duration = res.timings ? res.timings.duration : 0;
  const validation = validateReferralsResponse(res);

  // Map contract validations to K6 assertions checks
  const isOk = check(res, {
    'Referrals - status code is 200': (r) => r.status === 200,
    'Referrals - contract validations pass': () => validation.valid,
  });

  if (isOk) {
    // Record Success Metrics
    referralsSuccess.add(1);
    referralsFailureRate.add(false);
    referralsDuration.add(duration);
  } else {
    ...
    console.warn(
      `[REFERRALS FAIL] Phone: ${phone}, Status: ${res ? res.status : 'None'}, ` +
