/**
 * K6 Load Test Scenario - Sessions GET /api/sessions
 * Responsibility: Performs authentication for user accounts, queries user sessions list,
 * asserts core API payload contracts, and tracks sessions query performance SLAs.
 */
...
import { validateSessionsResponse } from '../utils/responseValidators.js';
import { sleepRandom } from '../utils/helpers.js';
import { generateSummary } from '../utils/reporter.js';
import { sessionsDuration, sessionsSuccess, sessionsFailure, sessionsFailureRate } from '../customMetrics/metrics.js';
...
export default function () {
  ...
  if (!account) {
    sessionsFailure.add(1);
    sessionsFailureRate.add(true);
  ...
  if (!auth.token || !auth.validation.valid) {
    sessionsFailure.add(1);
    sessionsFailureRate.add(true);
    console.warn(`[SESSIONS ABORT] Login authentication failed for phone ${phone}, skipping iteration.`);
    return;
  }
  ...
  try {
    ...
    sessionsFailure.add(1);
    sessionsFailureRate.add(true);
    console.error(`[NETWORK EXCEPTION] GET /sessions failed for Phone: ${phone}. Error: ${err.message}`);
    return;
  }

  const duration = res.timings ? res.timings.duration : 0;
  const validation = validateSessionsResponse(res);

  // Map contract validations to K6 assertions checks
  const isOk = check(res, {
    'Sessions - status code is 200': (r) => r.status === 200,
    'Sessions - contract validations pass': () => validation.valid,
  });

  if (isOk) {
    // Record Success Metrics
    sessionsSuccess.add(1);
    sessionsFailureRate.add(false);
    sessionsDuration.add(duration);
  } else {
    ...
    console.warn(
      `[SESSIONS FAIL] Phone: ${phone}, Status: ${res ? res.status : 'None'}, ` +
