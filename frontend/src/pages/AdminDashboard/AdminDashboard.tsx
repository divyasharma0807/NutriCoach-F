...
interface AdminDashboardProps { userName: string; onLogout: () => void; profileData?: any; }
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ userName, onLogout, profileData }) => {
  ...
  // My Coaches state
  const [coaches, setCoaches] = useState<{ id: string; name: string; clientsCount: number; sessionsCount?: number; prospectsCount?: number; coachesCount?: number; level: string; status?: 'active' | 'inactive' }[]>([]);
  ...
  const handleToggleCoachStatus = async (coachId: string) => {
    const coachObj = coaches.find(c => c.id === coachId);
    if (!coachObj) return;
    const currentStatus = coachObj.status || 'Active';
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
                  ...
                  />
                  <label 
                    htmlFor="admin-add-result-upload"
    ...
    if (currentSection === 'coach-performance') {
      return (
        <CoachPerformance
          ...
          referrals={referrals}
          results={results}
          onAddCoach={() => setIsAddCoachOpen(true)}
          onToggleStatus={(id) => handleToggleCoachStatus(id)}
          onDeleteCoach={(id) => { setItemToDelete({ type: 'coach', id }); setIsDeleteModalOpen(true); }}
        />
      );
    }
